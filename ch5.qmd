...
- validation set approach: estimate the test error by *holding out* a subset of the training set, also called a *validation set*. 
  ...
  * Only a subset of the training set is used to fit the model. This suggests the validation error tends to *overestimate* the test error compared to the model that uses the entire traning set. 
