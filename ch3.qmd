...
$${#eq-correlation-rxy}
is the normalized convariance. Note $-1\le r_{xy} \le 1$. When there is no intercept, that is $\beta_0=0$, then 
$$
...
$$
where, $\text{RSS}_0$ is the residual sum of squares of a second model that uses all variables *except* those last $q$ variables. When $q=1$, $F$-statistic in @eq-subset-hypothesis is the square of the $t$-statistic of that variable. 
...
$$
It is the *average amount* in  $Y$ that a response deviates from the *true regression line* ($\beta_0+\beta_1 X$).  Note, RSE can increase with more variables if the decrease of RSS doesnot offset the increase of $p$. 
